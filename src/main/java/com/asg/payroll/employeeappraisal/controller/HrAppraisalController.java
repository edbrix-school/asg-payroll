...
public class HrAppraisalController {
    ...
    public ResponseEntity<?> printA3(@PathVariable Long transactionPoid) {
        try {
            ...
            return ResponseEntity.ok()
                    .header(HttpHeaders.CONTENT_DISPOSITION, "attachment; filename=appraisal-a3-" + transactionPoid + ".pdf")
    ...
    public ResponseEntity<?> printByCompany(@PathVariable Long transactionPoid) {
        try {
            ...
            return ResponseEntity.ok()
                    .header(HttpHeaders.CONTENT_DISPOSITION, "attachment; filename=appraisal-by-company-" + transactionPoid + ".pdf")
                    .contentType(MediaType.APPLICATION_PDF)
                    .body(pdf);
        } catch (Exception e) {
            return error(FAILED_TO_GENERATE_PDF + e.getMessage(), 500);
    ...
    public ResponseEntity<?> printBank(@PathVariable Long transactionPoid) {
        try {
            ...
            return ResponseEntity.ok()
                    .header(HttpHeaders.CONTENT_DISPOSITION, "attachment; filename=appraisal-bank-" + transactionPoid + ".pdf")
                    .contentType(MediaType.APPLICATION_PDF)
                    .body(pdf);
        } catch (Exception e) {
            return error(FAILED_TO_GENERATE_PDF + e.getMessage(), 500);
